powered by 35's Embedded Systems Inc. A Next generation Driver Solution for Portable Ultra high Output Laser Pointer                                                                                                                     

### 概述

Xtern Ripper Laser Edition(简称XTRLaser)是专用于驱动日亚LD BANK **（例如NUBB23、NUBB33以及NUBB27、NUBB37等）** 的高能效侧按电子开关控制的升压恒流驱动器。该驱动器具备自适应功能，同时支持单锂和双锂输入并自动适配采用最合适的功率驱动激光模组。该驱动的尺寸为22mm，高度7mm，可以完美兼容东东海D2GC或M0AC等22mm驱动位的侧按小直外壳。在M0AC小直内搭配日亚NUBB23模块时可以实现高达31W的峰值光功率并且稳定运行至温度保护激活。是实现小体积暴脾气亮骚烧东西激光小直的专用解决方案。相比市面上基于LED手电筒驱动的魔改产物具有更高的耐压（高达25V Rated可以适配20V工作电压的五管LD Bank） **（例如那些基于MP3431芯片单双锂输入通用升压手电驱动改反馈电阻做的LD驱动，芯片容许的最高输出电压为16V然后NUBB33类的四管激光模块的Vf高达15.98V，长期顶着芯片的输出耐压极限跑就不怕炸管么？）** 和稳定性以及更适配和更智能的温度保护和操作逻辑（一键点动烧灼模式，自动锁定等）。

### 性能数据

+ 支持的电池输入类型：1S至2S的三元锂电池组，2S电池组的总交流内阻需小于50mΩ且额定放电能力大于等于15A。 **注意：驱动在1S模式下将会自动限制输出功率至约30W避免IC过载，但是此时仍然需要使用全极耳类型的低内阻动力电池，放电能力应大于20A且交流内阻小于8mΩ。如果需要获得完整的使用体验，请搭配2S电池组使用！**
+ 支持的LD Bank类型：（单/双锂）NUBB13、NUBB23、NUBB33 **NUBB27和NUBB37硬件理论支持，但是需要调整参数，目前暂未有条件测试** 。（仅单锂并搭配特定固件和硬件）NURM11T，具体请前往下方的LD Bank类型章节查看。
+ 支持的LD电流：150mA至4.1A（适配单管多串LD）
+ 温度保护：46度多阶变值PI环+常亮预测算法实现的自适应外壳恒温（温控启动后驱动根据外壳散热能力和环境自动调整LD的亮度达到最高可实现的亮度，充分利用外壳的散热能力和热容）+55度极亮cutoff（强制退出极亮并返回到低亮）+65度强制关机三段式温度保护。**相比那些温度一到就给你拉闸不给开机的断崖式温度保护极大提高了用户的使用体验。都5202年了还没有一个能实现自适应恒温的激光驱动，属实是扯蛋。**
+ 电池保护：具有欠压自动降档和关机保护以及过电压保护
+ 输出保护：具备LD开路和短路检测

### 输出效率

目前该驱动已进行了输出效率的初步测试，结果如下：

+ 双锂模式驱动NUBB23（全功率）：`输入：7.4V*9.5A=70.3W，输出15.99V*4.21A=67.3179W` 此时典型效率约96.01%，损耗2.98W
+ 单锂模式驱动NUBB23（半功率）：`输入：3.53V*8.90A=31.417W，输出14.93V*1.91A=28.5163W` 此时典型效率约90.76%，损耗2.90W

受限于较低的工作频率所需的4R7电感导致DCR发热损耗较大使得在单锂工作时效率不好看。不过该效率对于输出耐压支持20V LD Bank的专用22mm驱动而言仍然处于优秀水平，不需要担心在壳子内过热而无法工作。

### 自动锁定功能说明

考虑到该类激光手电具有极高的输出功率，当用户长时间静置未用，并且在某天突然拿起时会因为忘记手电处于何种状态并误开启高功率模式造成严重的安全事故。**激光贴吧之前就有因为熊孩子误拿起长期静置且未处于锁定模式的大功率激光手电并且未做任何防护的情况下意外开启造成眼睛永久失明的案例！！** 因此该驱动固件具备自动锁定功能，当手电检测到用户长时间未使用并静置超过20分钟后，驱动将会自动进入锁定模式并且重置挡位记忆到最低挡位确保安全。进入锁定模式后即使重新安装电池也无法解锁，此时用户需要参考下方章节关于按键锁的内容，手动解锁手电后手电才会恢复正常使用状态。且解锁手电后，首次单击开机时手电会固定进入低挡位以确保安全使用，用户可以自行换挡。
...
+ 无极调光模式：关机状态下长按进入。该挡位允许用户在0.4A至3A（对应0.5-22W光功率）的范围内无极调整并选定一个指定的亮度挡位（如果驱动工作在单锂模式，则调节范围将会被限制为0.4-1.8A对于0.5-12W光功率）。进入该挡位后长按升高亮度，单击+长按减少亮度。若亮度达到最高或最低时驱动会令LD慢闪提示您亮度调整已经达到限制。若要回到关机状态，则单击开关一次。 **注：该挡位的亮度数据具有永久性记忆，设定好亮度后约3秒系统会永久保存至存储器内，即使断电也不会丢失。**
+ 极亮挡位：任意状态下双击进入。如果您只安装了一节电池，则双击会进入到循环挡位中的中高档此时仍然能一键进入循环档中的最高功率（1.8A电流，对应12W光功率）模拟双击极亮的行为，**但是无法开启需要双锂的真正全功率极亮。** 如果您安装了2节电池，则此时驱动将会以全功率输出并实现约31W的光功率。 **该挡位发热巨大且输出功率极高，为了避免引起火灾请务必小心使用！**
